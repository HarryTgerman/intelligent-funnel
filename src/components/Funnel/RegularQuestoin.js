...
export default function RegularQuestoin(props) {
    ...
    const toggleSelectItem = async(selector, index) => {
        ...
        if(!isMultiselect && !selector.additional){
          let currentQuestionIndex = state.areas[selector.pathToArea].findIndex(i => i.id === selector.pathToQuestion);
          let currentArea =  selector.pathToArea;
          let nextQuestion = state.areas[selector.pathToArea].find(i => i.id === selector.pathToQuestion)
          ...
          // let history = [ ...state.history, { question: selector.parentId, area: state.currentArea }]
          await dispatch({ type: 'UPDATE_CARD', payload: {selector, index} })
          await dispatch({ type: 'UPDATE_STATE', payload: { currentQuestionIndex, currentArea, questions } })
        }
        else {
          await dispatch({ type: 'UPDATE_CARD', payload: {selector, index} })
